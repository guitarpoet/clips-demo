    ...
    $.fn.fullpage = function(options) {
        ...
        */
        ...
        FP.moveSectionUp = function(){
			...
			if(!prev.length) {
				if( !options.beforeScroll && $(options.beforeElement).css('position') != 'absolute' && $(options.beforeElement).css('position') != 'fixed' ) {
					prev = $(options.beforeElement);
					options.beforeScroll = true;
        ...
        FP.moveSectionDown = function (){
			...
			if($(options.beforeElement).css('position') != 'absolute' && $(options.beforeElement).css('position') != 'fixed' ) {
				if(options.beforeScroll) {
					options.beforeScroll = false;
					next = $(SECTION_ACTIVE_SEL);
        ...
        $(SECTION_SEL).each(function(index){
			...
			if($(options.beforeElement).length > 0) {
				if ($document.scrollTop() >= $(options.beforeElement).outerHeight()) {
					options.beforeScroll = false;
				} else {
					options.beforeScroll = true;
					//$document.scrollTop(0);
					//$htmlBody.scrollTop(0);
					scrollPage($('.section').eq(0));
				}
			}
        ...
        */
        function scrollPage(element, callback, isMovementUp){
			...
			if($(options.beforeElement).length > 0) {
				if(!element.is($(options.beforeElement)) && $(options.beforeElement).css('position') != 'absolute' && $(options.beforeElement).css('position') != 'fixed' ) {
					v.dtop += $(options.beforeElement).outerHeight();
				}
			}
